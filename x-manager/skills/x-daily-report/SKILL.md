...
### ステップ1: メインエージェント — データ取得（SQL 2本を並列実行）

以下の2つのSQLを `execute_sql` で**並列実行**する。

#### SQL-A: 投稿パフォーマンス（上位20件）
...
**promptに含める内容：**
1. SQL-A結果データ（投稿パフォーマンス、JSON文字列としてそのまま貼り付け）
2. SQL-B結果データ（フォロワー推移、JSON文字列としてそのまま貼り付け）
3. agents/x-daily-analyzer.md に定義されたサブエージェント指示に従って処理を実行するよう指示

### ステップ3: メインエージェント — 完了報告（厳守）

サブエージェントの返答から `REPORT_PATH=` で始まる行を探し、そのパスを取得する。

1. **Readツールでレポートファイルを読み込む**（このステップは絶対にスキップしない）
   - パスは必ず `/mnt/c/Users/tomoh/Dropbox/Cursor/cowork/cowork_plugin/x-manager/log/x-daily-report_YYYY-MM-DD.md` になる
   - Readツールで読み込むことで、Claude Code上で折りたたみ式に表示され、ユーザーがクリックで閲覧可能になる
