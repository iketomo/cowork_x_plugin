...
description: >
  ...
  Supabase Edge Function `x-generate-article-image` を呼び出し、
  日本のビジネス書風「ゆるいイラスト」スタイルの横長画像を生成してSupabase Storageに保存し、公開URLを返す。
...
Supabase Edge Function (`x-generate-article-image`) 経由で Nano Banana 2（Gemini AI）を使って記事用横長画像を生成する。

## 前提条件

- Supabase に Edge Function `x-generate-article-image` がデプロイ済み（モデル: `gemini-3.1-flash-image-preview` / Nano Banana 2）
...
- URL形式: `https://{project}.supabase.co/storage/v1/object/public/x-images/YYYY/MM/x_article_image_YYYYMMDDHHMMSS_{suffix}.png`
