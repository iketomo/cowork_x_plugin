...
以下のバージョンは **常に全プラグインで同一の値** を維持する：

1. `.claude-plugin/marketplace.json` の各 `plugins[].version`
2. 各プラグイン直下の `.claude-plugin/plugin.json` の `version`

### 更新手順

1. CLAUDE.md の内容を更新
2. バージョン番号を `1.0.X` → `1.0.X+1` のように段階的にアップ
3. marketplace.json の全プラグインの version を更新
4. 各 plugin.json の version を更新
5. 全てが同一バージョンであることを確認
