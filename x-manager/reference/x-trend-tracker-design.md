...
### 目的
AI・生成AI領域でバズっている投稿を日次で自動収集・分析し、自身のX投稿戦略に活かすためのインサイトを毎日Slackに配信する。
...
> ※ 以下は設定例です。実際の追跡アカウントは `x_trend_accounts` テーブルで管理されます。
...
Webhook URLは `config.local.md` の Slack 設定を参照。
